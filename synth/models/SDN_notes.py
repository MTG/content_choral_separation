...
from synth.data_pipeline import SDN_notes as data_pipeline
...
class SDN(model.Model):
    ...
    def __init__(self):
        ...
        self.get_optimizers()
        self.get_summary(self.sess, config.SDN_notes_log_dir, summary_dict)
    ...
    def load_model(self, sess):
        ...
        self.stft_var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_encoder') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_decoder') \
        + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_post_net') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = 'F0_Model') \
        + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = 'Vuv_Model') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = 'Notes_Model')
        ...
        ckpt_stft = tf.train.get_checkpoint_state(config.SDN_notes_log_dir)
    ...
    def get_optimizers(self):
        ...
        self.notes_optimizer = tf.train.AdamOptimizer(learning_rate = config.init_lr)
        ...
        self.global_step_notes = tf.Variable(0, name='global_step_notes', trainable=False)
        ...
        self.notes_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = 'Notes_Model')
        ...
        with tf.control_dependencies(update_ops):
            ...
            self.notes_train_function = self.notes_optimizer.minimize(self.notes_loss, global_step=self.global_step_notes, var_list=self.notes_params)
    ...
    def loss_function(self):
        ...
        self.notes_loss = tf.reduce_sum(tf.abs(self.notes - self.notes_placeholder))
        ...
        summary_dict = {"recon_loss" : self.recon_loss, "content_loss": self.content_loss, "recon_loss_0": self.recon_loss_0, "final_loss": self.final_loss,\
         "f0_loss": self.f0_loss, "vuv_loss": self.vuv_loss, "notes_loss": self.notes_loss}
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, config.SDN_num_epochs):
            ...
            with tf.variable_scope('Training'):
                for feats_targs, stft_targs, targets_speakers, notes_targs in data_generator:


                    final_loss, recon_loss, recon_loss_0, content_loss, f0_loss, vuv_loss, notes_loss, summary_str = self.train_model(feats_targs, stft_targs, targets_speakers, notes_targs, self.sess)
                ...
                epoch_notes_loss = epoch_notes_loss/batch_num
                ...
                print_dict["Notes Loss"] =  epoch_notes_loss


            batch_num = 0
            with tf.variable_scope('Validation'):
                for feats_targs, stft_targs, targets_speakers, notes_targs in val_generator:


                    final_loss, recon_loss, recon_loss_0, content_loss, f0_loss, vuv_loss, notes_loss, summary_str = self.validate_model(feats_targs, stft_targs, targets_speakers, notes_targs, self.sess)
                ...
                val_notes_loss = val_notes_loss/batch_num
                ...
                print_dict["Val Notes Loss"] =  val_notes_loss
            ...
            if (epoch + 1) % config.save_every == 0 or (epoch + 1) == config.SDN_num_epochs:
                self.save_model(self.sess, epoch+1, config.SDN_notes_log_dir)


    def train_model(self,feats_targs, stft_targs, targets_speakers, notes_targs, sess):
        """
        ...
        _,_,_,_, final_loss, recon_loss, recon_loss_0, content_loss, f0_loss, vuv_loss, notes_loss = sess.run([self.final_train_function, self.f0_train_function, self.vuv_train_function, self.notes_train_function,\
         self.final_loss, self.recon_loss, self.recon_loss_0, self.content_loss, self.f0_loss, self.vuv_loss, self.notes_loss], feed_dict=feed_dict)

        summary_str = sess.run(self.summary, feed_dict=feed_dict)


        return final_loss, recon_loss, recon_loss_0, content_loss, f0_loss, vuv_loss, notes_loss, summary_str
 

    def validate_model(self,feats_targs, stft_targs, targets_speakers, notes_targs, sess):
        """
        Function to train the model for each epoch
        """
        ...
        final_loss, recon_loss, recon_loss_0, content_loss, f0_loss, vuv_loss , notes_loss = sess.run([self.final_loss, self.recon_loss, self.recon_loss_0, self.content_loss, self.f0_loss, self.vuv_loss, self.notes_loss], feed_dict=feed_dict)
    ...
    def test_file_wav(self, file_name, speaker_index):
        ...
        if synth:
            ...
            sf.write(os.path.join(config.output_dir,'{}_{}_SDN_notes.wav'.format(file_name[:-4], config.singers[speaker_index])), audio_out, config.fs)
    ...
    def model(self):
        ...
        with tf.variable_scope('Notes_Model') as scope:
            self.notes = modules_SDN.enc_dec_f0(self.stft_placeholder, self.output_stft[:,:,:-4], self.output_stft[:,:,-4:], self.is_train)

        with tf.variable_scope('F0_Model') as scope:
            self.f0 = modules_notes.f0_model(self.content_embedding_stft, self.notes, self.speaker_onehot_labels_1, self.is_train)
